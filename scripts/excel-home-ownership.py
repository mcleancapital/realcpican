...
EXCEL_FILE_PATH = './data/home-ownership.xlsx'

# URL and headers
URL = "https://tradingeconomics.com/canada/home-ownership-rate"
HEADERS = {
...
def fetch_recent_lei_data(url):
    ...
    try:
        ...
        # Extract the 7th numeric value for the Average Home Price
        numeric_values = []
        for td in td_elements:
            text = td.get_text(strip=True)
            if text.replace('.', '', 1).isdigit():  # Check if the text is a valid number
                numeric_values.append(float(text))
            if len(numeric_values) == 7:  # Stop once we have the 7th numeric value
                break
