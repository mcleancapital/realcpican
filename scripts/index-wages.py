...
def update_sp500_html(html_file, excel_file, output_file):
    try:
        ...
        # Drop rows where 'Date' or "% Change vs Last Year" is missing
        df = df.dropna(subset=["Date", "% Change vs Last Year"])
        ...
        value_array = df["% Change vs Last Year"].tolist()
        ...
        # Get the most recent date and "% Change vs Last Year"
        most_recent_date = df.iloc[-1]["Date"]
        most_recent_value = df.iloc[-1]["% Change vs Last Year"]
        ...
        sp500_marker = '<a class=box href="/wages">'
