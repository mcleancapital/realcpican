...
EXCEL_FILE_PATH = './data/finance.xlsx'

# URL and headers
URL = "https://tradingeconomics.com/canada/households-debt-to-income"
HEADERS = {
...
def fetch_recent_lei_data(url):
    """Fetch the 1st numeric value and the date from the 9th <td> element."""
    try:
        ...
        for td in td_elements:
            ...
            if len(numeric_values) == 9:  # Stop once we have the 9th numeric value
                break
        ...
        if len(td_elements) < 24:
            raise Exception("Less than 11 <td> elements found.")
        recent_date_str = td_elements[24].get_text(strip=True)
...
def update_excel(file_path, recent_date, recent_value):
    ...
    try:
        ...
        for row in range(2, ws.max_row):
            ws.cell(row=row, column=3, value=f"=(B{row}/B{row+4}-1)*100")
